...
class Mirasvit_Helpdesk_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    /*
     * Translates extension's csv file into Javascript-friendly hash
     */
        ...
        if (!$this->_jstranslation) {
            try {
                ...
                $translator = new Zend_Translate(
                    array(
                        'adapter' => 'csv',
                        'content' => Mage::getBaseDir('locale').'/'.$locale.'/'.'Mirasvit_Helpdesk.csv',
        ...
        if (Mage::getVersion() <= '1.4.1.1') {
            ...
            foreach ($collection as $address) {
                $result[] = array(
                    ...
                    'order_id' => $address->getOrderId(), /* @bug ??? it seems $address doesn't have this property */
            ...
            foreach ($collection as $address) {
                ...
                $result[] = array(
                    'id' => 'address_'.$address->getId(),
                    'order_id' => $address->getOrderId(), /* @bug ??? it seems $address doesn't have this property. it has 'parent_id' */
                    'name' => $address->getFirstname().' '.$address->getLastname().' ('.$address->getEmail().') [unregistered]',
                    'email' => $address->getEmail(),
                );
            }
