...
class Mirasvit_Helpdesk_Adminhtml_TemplateController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $this->_addContent($this->getLayout()
            ->createBlock('helpdesk/adminhtml_template'));
        $this->renderLayout();
    }
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $template = Mage::getModel('helpdesk/template');
        ...
        if (!is_array($ids)) {
            ...
            try {
                ...
                foreach ($ids as $id) {
                    ...
                    $template = Mage::getModel('helpdesk/template')->load($id);
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('helpdesk/dictionary/template');
    }
