...
class Mirasvit_Helpdesk_Helper_String extends Varien_Object
{
    ...
    public function getTicketCodeFromSubject($subject)
    {
        ...
        //aw tickets
        if (($this->getConfig()->getGeneralAcceptForeignTickets() == Mirasvit_Helpdesk_Model_Config::ACCEPT_FOREIGN_TICKETS_AW) &&
                $subject && preg_match('[[#][A-Z]{1,3}-[0-9]{1,5}]', $subject, $regs)) {
            return ltrim($regs[0], '#');
        }

        //mw tickets
        if ((Mage::getSingleton('helpdesk/config')->getGeneralAcceptForeignTickets() == Mirasvit_Helpdesk_Model_Config::ACCEPT_FOREIGN_TICKETS_MW) &&
...
//@dva fix end
     ...
     * @return string
     */
    protected function _preg_pre_callback(/* @noinspection PhpUnusedParameterInspection */ $matches)
    {
